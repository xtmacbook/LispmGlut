...
int showHelp = 0;
bool drawFrustumBe = false; // be added by xt
...
{
	static GLuint F_SCENE = 0;
	
	if(drawFrustumBe)
	{
		drawFrustumBe = !drawFrustumBe;
		struct Object obj = OBJECT_NULL;
		testFrustum(&obj);
		F_SCENE = glGenLists(1);
        glNewList(F_SCENE, GL_COMPILE);
    ...
    glBegin(GL_LINES);
		...
		for(unsigned int j = 0; j < p->size; j++) {
			int idNext = (j+1) % p->size;
			
			glVertex3f(p->points[j][0],p->points[j][1],p->points[j][2]);
			glVertex3f(p->points[idNext][0],p->points[idNext][1],p->points[idNext][2]);
            }
        }
    glEnd();
    glEndList();
...
void display(void) {
	const char *msg1 = 0, *msg2 = 0, *msg3 = 0;
